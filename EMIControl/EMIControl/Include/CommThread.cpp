...
BOOL CCommThread::OpenPort(CString strPortName, DWORD dwBaud, BYTE byData, BYTE byStop, BYTE byParity )
{
	// Local 변수.
		...
		//--> Read 이벤트 생성에 실패..
		if ( !(m_osRead.hEvent = CreateEventW(NULL, TRUE, FALSE, NULL)) ) 	
		{
			return FALSE;
		}
		...
		//--> Write 이벤트 생성에 실패..
		if (! (m_osWrite.hEvent = CreateEventW(NULL, TRUE, FALSE, NULL)))
		{
			return FALSE;
		}
		...
		if (m_hComm == (HANDLE) -1)
		{
			AfxMessageBox(_T("Fail to Open Comm Port!"));
			return FALSE;
		}
...
BOOL CCommThread::WriteCommBlock(LPSTR lpByte , DWORD nToWrite)
{
	...
	if(!WriteFile(m_hComm, lpByte, nToWrite, &dwWritten, &m_osWrite))
	{
		
		if(GetLastError() == ERROR_IO_PENDING){				// 아직 전송할 문자가 남았을 경우..
			OutputDebugString(_T("\n\rIO Pending"));
			// 읽을 문자가 남아 있거나 전송할 문자가 남아 있을 경우 Overapped IO의 특성에 따라 ERROR_IO_PENDING 에러 메시지가 전달된다. timeout에 정해준 시간만큼 기다려준다.
...
//포트에 pBuff의 내용을 nToWrite만큼 쓴다. 실제로 쓰여진 Byte수를 리턴한다.
DWORD CCommThread::WriteComm(BYTE *pBuff, DWORD nToWrite)
{
	...
	if(!WriteFile(m_hComm, pBuff, nToWrite, &dwWritten, &m_osWrite))
	{
		...
		if (GetLastError() == ERROR_IO_PENDING)		{
			// 읽을 문자가 남아 있거나 전송할 문자가 남아 있을 경우 Overapped IO의 특성에 따라 ERROR_IO_PENDING 에러 메시지가 전달된다. time out에 정해준 시간만큼 기다려준다.
			while (! GetOverlappedResult( m_hComm, &m_osWrite, &dwWritten, TRUE))
			{
...
DWORD CCommThread::ReadComm(BYTE *pBuff, DWORD nToRead)
{
	...
	CString returndata;
	CString teststring((char*)pBuff);

	if(dwRead!=0){
		returndata = teststring.Left(dwRead);
...
// 포트로부터 pBuff에 nToWrite만큼 읽는다.
// 실제로 읽혀진 Byte수를 리턴한다.
CString CCommThread::GetReturn(BYTE *pBuff, DWORD nToRead)
{
// 	DWORD           dwEvent;
// 	OVERLAPPED      os;
// 	BOOL            bOk = TRUE;
...
// 			do
// 			{
// 				dwRead = pComm->ReadComm( buff, 4096); //들어온 데이터 읽어 오기 
...
DWORD	ThreadWatchComm(CCommThread* pComm)
{
   ...
   //--> 이벤트 설정..
   if( !(os.hEvent = CreateEventW( NULL, TRUE, FALSE, NULL)) )
   {
		bOk = FALSE;
   }
