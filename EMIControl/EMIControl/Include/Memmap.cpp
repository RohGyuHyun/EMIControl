...
BOOL CMemMapFile::MapFile(const CString& sFilename, BOOL bReadOnly, DWORD dwShareMode, BOOL bAppendNull, BOOL bNamed, BOOL bGrowable, DWORD dwStartOffset, DWORD dwNumberOfBytesToMap, LPSECURITY_ATTRIBUTES lpSecurityAttributes)
{
...
BOOL CMemMapFile::MapMemory(const CString& sName, DWORD dwBytes, BOOL bReadOnly, LPSECURITY_ATTRIBUTES lpSecurityAttributes)
{
  m_dwLength = dwBytes;
  m_bReadOnly = bReadOnly;
  m_bAppendNull = FALSE;
...
BOOL CMemMapFile::MapHandle(HANDLE hHandle, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwStartOffset)
{
  ...
  m_hMapping = ::CreateFileMapping(hHandle, lpSecurityAttributes, flProtect, 0, dwLength, m_sMappingName);
  if (m_hMapping == NULL)
  {
